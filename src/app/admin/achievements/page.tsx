...
export default function AdminAchievementsPage() {
  ...
  async function moveItem(id: string, dir: -1 | 1) {
    ...
    await Promise.all([
      ...
      fetch(`/api/achievements/${other._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ order: arr[idx].order }),
      }),
    ]);
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span
                    className={`font-mono text-[10px] capitalize ${TYPE_COLOR[a.type]}`}
                  >
                    {a.type}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={() => moveItem(a._id, -1)}
                  disabled={items.indexOf(a) === 0}
                  className="text-dim hover:text-text disabled:opacity-20 transition-colors p-1 font-mono text-[10px]"
                >
                  ▲
                </button>
                ...
                <button
                  onClick={() => moveItem(a._id, 1)}
                  disabled={items.indexOf(a) === items.length - 1}
                  className="text-dim hover:text-text disabled:opacity-20 transition-colors p-1 font-mono text-[10px]"
                >
                  ▼
                </button>
